...
class Model:
    ...
    def build(self, action_dim=7):
        ...
        with tf.variable_scope(self.scope):
            ...
            fc3 = tf.layers.dense(fc2, 512, name='over_fc3', activation=tf.nn.relu, use_bias=False)
            fc3 = tf.contrib.layers.layer_norm(fc3, begin_norm_axis=-1)
            fc4 = tf.layers.dense(fc3, 512, name='over_fc4', activation=tf.nn.relu, use_bias=False)
            ...
            fc2 = tf.contrib.layers.layer_norm(fc2, begin_norm_axis=-1)
            fc3 = tf.layers.dense(fc2, 512, name='under_fc3', activation=tf.nn.relu, use_bias=False)
            fc3 = tf.contrib.layers.layer_norm(fc3, begin_norm_axis=-1)
            fc4 = tf.layers.dense(fc3, 512, name='under_fc4', activation=tf.nn.relu, use_bias=False)
            fc4 = tf.contrib.layers.layer_norm(fc4, begin_norm_axis=-1)
            ...
            fc2 = tf.contrib.layers.layer_norm(fc2, begin_norm_axis=-1)
            fc3 = tf.layers.dense(fc2, 512, name='whole_fc3', activation=tf.nn.relu, use_bias=False)
            fc3 = tf.contrib.layers.layer_norm(fc3, begin_norm_axis=-1)
            fc4 = tf.layers.dense(fc3, 512, name='whole_fc4', activation=tf.nn.relu, use_bias=False)
            fc4 = tf.contrib.layers.layer_norm(fc4, begin_norm_axis=-1)
            self.feature_whole_2 = fc4

            over_feature_pool = tf.reduce_max(self.feature_over_2, axis=1)
            under_feature_pool = tf.reduce_max(self.feature_under_2, axis=1)
            whole_feature_pool = tf.reduce_max(self.feature_whole_2, axis=1)
            self.state_feature = tf.concat([over_feature_pool, under_feature_pool, whole_feature_pool], axis=-1)
            ...
            fc1 = tf.concat([self.state_feature, fc1_a], axis=-1)
            ...
            # state value
            fc1_v = tf.layers.dense(self.state_feature, 256, name='fc1_v', activation=tf.nn.relu)
    ...
    def predict_single(self, sess, obs, over_seg_dict, under_seg_dict, action):
        ...
        action = np.concatenate([obs[action_node,:2], action[1:]])
    ...
    def predict_batch(self, sess, obs, over_seg_dict, under_seg_dict, action):
        ...
        action_node = (action[:,0]*63).astype(np.int32)
        action_node = np.clip(action_node, 0, 63)
        action = np.concatenate([obs[np.arange(len(obs)), action_node, :2], action[:,1:]], axis=-1)
    ...
    def predict_single_action(self, sess, obs, over_seg_dict, under_seg_dict,
        ...
        for iter in range(iterations):
            ...
            feed_dict = {self.state_feature:np.tile(state_feature, (CEM_population, 1)),
                         self.action:convert_action_samples}
    ...
    def predict_batch_action(self, sess, obs, over_seg_dict, under_seg_dict,
        ...
        for ob, feat, action_samples in zip(obs, state_feature, init_action_samples):
            for iter in range(iterations):
                ...
                feed_dict = {self.state_feature:np.tile(feat, (CEM_population, 1)),
                             self.action:convert_action_samples}
                qs = sess.run(self.q_value, feed_dict=feed_dict)
                idx = np.argsort(qs[:,0])
    ...
    def fit_q(self, sess, obs, over_seg_dict, under_seg_dict, actions, q_targets):
        feed_dict = self.make_feed_dict_batch(obs, over_seg_dict, under_seg_dict)
        action_node = (actions[:,0]*63).astype(np.int32)
        action_node = np.clip(action_node, 0, 63)
        actions = np.concatenate([obs[np.arange(len(obs)), action_node, :2], actions[:,1:]], axis=-1)
